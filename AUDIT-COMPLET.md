...
| **Plafond de durée totale** | `_audit-catalog.mjs` `MAX_TOTAL_RUN_TIME_MS = 60 min` | Kill du process (exit 124), compté en **temps éveillé**. Valeur choisie sur mesure : un run complet chronométré le 21.08.2026 prend **33 min** pour 1058 offres (57 min avant l’allègement des replis ; 23 à 30 min les jours précédents). Le budget vaut donc ~2× le réel. Historique : 30 min tronquait des runs sains toujours en fin de liste (travel/prepaid/dataOnly) sans jamais le signaler ; 180 min laissait un vrai blocage tourner trois heures. Deux choses rendent la baisse sûre : le budget ignore la veille système, et ce n’est plus lui qui attrape les blocages. |
| **Watchdog de progression** | `_audit-catalog.mjs` `MAX_STALL_MS = 6 min` de temps éveillé (19.08.2026) | Kill du process (exit **125**, distinct du 124) si **aucune offre ne se termine** pendant 6 min. C'est le garde-fou qui manquait : un run peut être vivant et figé, donc invisible pour un plafond de durée. Le 19.08, un run est resté 39 min sur une seule offre — sous les 3 h, donc jamais coupé. « Ce run est long » n'est pas un défaut ; « ce run n'avance plus » en est un, et la plus lente des offres prend 30 s. |
